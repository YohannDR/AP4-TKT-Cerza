...
exports.findAllCompl = async(req, res, next) => {
    ...
    try {
        //on stock le resultat de la requête
        const [rows] = await conn.execute(
            "SELECT mission.id, mission.libelle, description, complete, idUser, commentaire, date from mission", [req.body]
        );
        if (rows.length > 0) {
            return res.json({
                mission: rows
...
exports.findOneCompl = async(req, res, next) => {
    try {
        const [rows] = await conn.execute("SELECT * from mission WHERE mission.id =?", [
            req.params.id
        ]);
        if (rows.length > 0) {
            return res.json({
...
exports.assigner = async(req, res, next) => {
    try {
        const [rows] = await conn.execute('UPDATE `mission` SET idUser=? WHERE id=?', [
            req.body.idUser,
            req.params.id
        ]);
        if (rows.affectedRows === 1) {
